...
namespace BalanceMonitor.Infrastructure.Core
{
  ...
  {
    private readonly IEventBus eventPublisher;
    private readonly ISessionFactory sessionFactory;

    public EventStoreMsSql(ISessionFactory sessionFactory, IEventBus eventBus)
    {
      this.sessionFactory = sessionFactory;
      this.eventPublisher = eventBus;
    }

    public IEnumerable<IEsDomainEvent> GetEvents(Guid id)
    {
      var aggregateEvents = new List<IEsDomainEvent>();
      ...
      using (var ctx = session.Open())
      {
        ...
        foreach (var dbEvt in dbEvents)
        {
          ...
          if (deserialedEvent is IEsDomainEvent)
          {
    ...
    public void Save(IEsAggregateRoot ar)
    {
    ...
    private Aggregate GetAggregateFromStore<TAggregate>(TAggregate ar, BalanceMonitorEntities ctx) where TAggregate : IEsAggregateRoot
    {
    ...
    /// <returns></returns>
    private string SerializeEvent<TEvent>(TEvent @event) where TEvent : IEsDomainEvent
    {
